...
The first run opens a native window. Open the **Tests** tab, pick the model you pulled, and run a built-in agentic collection to get your first **Ready / Conditional / NotReady** verdict — then check the **Agent Report** tab for the per-model breakdown. (Editing `frontend/src/App.tsx` and saving triggers HMR.)
