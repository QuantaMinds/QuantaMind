...
/// the app did NOT start. Thin delegate to the Tauri-free domain client
/// (`inference::llama::llama_props`), which the lean `qm` CLI shares.
pub async fn probe_running_model(timeout_ms: u64) -> Option<(String, u32)> {
    crate::inference::llama::llama_props::probe_props(endpoint::LLAMA_SERVER, timeout_ms).await
}
