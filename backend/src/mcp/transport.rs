...
//! the stderr tail).
//!
//! The child leads its own process group (`process_group(0)` on unix,
//! `CREATE_NEW_PROCESS_GROUP` via `Host` on Windows) so teardown reaps the whole
//! tree — an MCP server is `npx` spawning `node`, and killing only the pid we hold
//! would orphan the grandchild. The group stop/kill lives in `os::proc_group`.
