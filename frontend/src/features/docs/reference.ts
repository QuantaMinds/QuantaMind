...
export const REFERENCE_SECTIONS: ReferenceSection[] = [
...

  {
    id: "cli",
    title: "CLI (qm)",
    blurb: "The headless face of QuantaMind — every command, with full syntax, mapped to the app page it mirrors.",
    blocks: [
      {
        id: "cli-overview",
        heading: "What the CLI is",
        what: "A `qm` binary sharing this app's exact eval engine: `doctor` (diagnose backends), `init` (zero-config first run), `run` (built-in suite → Ready/Conditional/NotReady verdict), `test` (your own collection file → per-mode scoreboard), `report` (re-assess a saved run offline), `cliff` (the Context Stress Test, headless).",
        why: "CI can't click a GUI. The CLI turns the readiness verdict into an exit code a pipeline can gate on, and lets air-gapped/SSH boxes run the same evals as this app — one scoring source of truth, so the CLI and the GUI can never disagree.",
        how: "Build once with `cargo build --bin qm` (a second binary on the same crate) and run `target/debug/qm <command>`. Env vars: QM_BASE (endpoint URL), QM_MODEL (model name), QM_API_KEY (remote bearer key — env/keychain only, never argv). `qm init` writes a `qm.json` in the cwd; later commands read it, so flags become optional. Exit-code contract, everywhere: 0 ready/ok · 10 conditional · 11 inconclusive (couldn't measure — retry) · 20 not-ready · 2 bad args · 3 unreachable/no runnable backend. Full reference: docs/cli/README.md in the repo.",
        formula: "qm <doctor|init|run|test|report|cliff> [flags]\nqm <command> --help        # per-command flag reference",
        source: "backend/src/cli/ · backend/src/bin/qm/main.rs",
      },
      {
        id: "cli-doctor",
        ...
        formula: "qm doctor                                  # scan all five backends\nqm doctor --backend <ollama|llama_cpp|mlx|vllm|sglang>\n          [--base <url>]     # endpoint override (env QM_BASE)\n          [--model <name>]   # also probe native tool-calling (env QM_MODEL)\n          [--json]           # report → stdout, fixes → stderr\n# exit: 0 = at least one backend is RUNNABLE · 3 = nothing runnable · 2 bad args",
      ...
      },
      {
        ...
        formula: "qm init [--json]\n# writes ./qm.json, then runs the suite; exit follows the verdict (0/10/20) or 3",
      ...
      },
      {
        ...
        formula: "qm run [--backend <kind>] [--model <name>] [--collection <id|file.json>]\n       [--mode <prompt_based|native|both>]      # default prompt_based\n       [--tier <easy|medium|hard|extreme>]      # difficulty override\n       [--thinking <lean|standard|deep>]        # lean = reasoning OFF\n       [--k <n>]                                # strict pass^k run count\n       [--profile <general-agent|rag-assistant|coding-agent|file.json>]\n       [--fail-on <conditional|notready|never>] # CI gate policy\n       [--junit <path>] [--save-report <path>] [--json]\n# exit: 0 Ready · 10 Conditional · 11 Inconclusive · 20 NotReady · 2 bad args · 3 unreachable",
      ...
      },
      {
        ...
        formula: "qm test --collection <file.json> [--backend <kind>] [--model <name>]\n        [--mode both] [--tier <t>] [--thinking <t>] [--k <n>]\n        [--fail-on <policy>] [--junit <path>] [--save-report <path>] [--json]",
      ...
      },
      {
        ...
        formula: "qm cliff [--backend <kind>] [--model <name>] [--collection <id|file>]\n         [--max-tokens <n>]   # deepest rung target (default 4096)\n         [--steps <k>]        # ladder rungs incl. baseline (default 4)\n         [--source <corporate_policy|system_logs|financial_ledger>]\n         [--json]\n# exit: 0 no-cliff · 10 collapsed · 11 inconclusive · 20 broken baseline",
      ...
      },
      {
        ...
        formula: "qm run … --save-report run.json          # 1) measure once\nqm report --report run.json \\\n          [--profile <id|profile.json>] \\\n          [--fail-on <policy>] [--junit <path>] [--json]\n# exit: the verdict (0/10/20) · 2 bad report/profile file",
