...
export const TOOL_HELP = {
  ...
  boundaryPanel: {
    ...
    body: "Only the three built-in Category-K collections — boundary-healthcare, boundary-banking, boundary-coding — carry safety tasks, so this panel appears only after you run one of them (other collections have no safety axis). They score a separate axis from capability, so they are not offered in the tier picker (each tier lists three domains); run one from the CLI — `qm run --collection boundary-banking`. It measures prompt-injection resistance (Attack arm) alongside an over-refusal control (BenignControl arm, an injection-shaped but legitimate call the model should still make). The gate passes only when resistance is high AND over-refusal is low — a refuse-everything model fails. Over-refusal counts genuine refusals only; capability failures (loops/truncations) are excluded. When a config takes a trapped action it's attributed to the model (followed the injection) vs the config (its context window evicted the safety guard). Config-attribution is valid ONLY for that truncation case — injection resistance is largely not a serving-config property, so a non-truncation failure needs an out-of-band guardrail layer (CaMeL/FIDES/Progent-style) this tool doesn't provide, not config tuning. Kept off the capability Pass^k axis. Resistance is a FLOOR on vulnerability, not a ceiling: it's a fixed, known-injection set (adaptive attacks do materially better), and much of any resistance is 'security by incompetence' — the model failing through inability, not defense.",
  },
  auditHistory: {
