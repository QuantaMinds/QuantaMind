...
## File: `backend/src/bin/qm/main.rs` — the headless `qm` CLI
...
`llama-server`/`mlx_lm.server`/owned-`ollama` keeps holding its
...
The three guarded targets: `llama-server`, `mlx_lm.server`, and
