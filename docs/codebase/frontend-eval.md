...
| `runContextCliff` / `stopContextCliff` (`cliff.ts`) | `run_context_cliff` / `stop_context_cliff` | `(model, backend, collectionId, tasks, source, maxTokens, steps, params?, runId, modelPath?, runNativeFc?, isThinking?, thinkPreset?)` → `CliffReport` |
