...
| **PerformanceMatrix** | One row per model: Pass^k, native FC, avg-steps, effort, schema-resil, **context limit**, top-error. Renders **BoundaryPanel** below the matrix when the collection carries Category-K tasks | reads `report`; pre-fills cliff | `batchStore`, `cliffStore` |
| **BoundaryPanel** *(Category K)* | Per config: resistance + over-refusal (honest em-dash when an arm didn't run), Pass/Fail/Inconclusive gate, model/config/unattributed attribution split, per-vector breakdown, and the static-set caveat (rendered prominently). Kept off the capability axis | reads `AggAgentic.boundary` off `report` | — |
