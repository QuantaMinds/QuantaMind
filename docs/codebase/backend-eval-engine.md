...
| **Context Stress Test** | `run_context_cliff` | Largest verified prompt-token depth before accuracy collapses | per-rung composite vs baseline; per-rung output budget = answer floor + depth-banded thinking scratchpad (`cliff/budget.rs::CliffBudget`, reuses `think_tokens_for_preset`); every rung carries an uncapped per-task tally (`TaskTally`/`by_task`); `Collapsed` = 20pp margin AND Newcombe interval excludes zero (`cliff/stats.rs`, else `Inconclusive`), with an exact-p + LOTO concentration label riding on the persisted status; per-cell decoded/thinking/cap-hit capture → amber near-cap + red cap-hit flags and the `BudgetLimited` status (all-cap-hit failures, checked before the collapse gate) |
