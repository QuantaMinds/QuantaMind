...
| **Context Stress Test** | `run_context_cliff` | Largest verified prompt-token depth before accuracy collapses | per-rung composite vs baseline; per-rung output budget = answer floor + depth-banded thinking scratchpad (`cliff/budget.rs::CliffBudget`, reuses `think_tokens_for_preset`; `flat_cap` — CLI `--cap` — pins one cap at every rung as the experimental control, since a cap that rises with depth co-varies with the variable under test); every rung carries an uncapped per-task tally (`TaskTally`/`by_task`); `Collapsed` = 20pp margin AND Newcombe interval excludes zero (`cliff/stats.rs`, else `Inconclusive`), with an exact-p + LOTO concentration label riding on the persisted status; per-cell decoded/thinking/cap-hit capture → amber near-cap + red cap-hit flags and the `BudgetLimited` status (all-cap-hit failures, checked before the collapse gate); a `finish == "length"` cell can NEVER score a pass (gated in `run_position` before the answer-key match; folds into died-at-cap — well-formed ≠ complete); `CapMarginal` = the baseline's tightest passing cell used ≥900‰ of the cap → refused at rung 0 before any padded rung is paid for (`cap_marginal`, one source for the run-loop stop AND `classify`) |
