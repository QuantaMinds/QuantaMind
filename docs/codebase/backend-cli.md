...
- Every number is **measured or `n/a`** — thinking tokens print `(tokenized split)`
  only when llama.cpp's `/tokenize` measured the split; llama.cpp prints `n/a`
