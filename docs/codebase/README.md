...
| [backend-inference-backends.md](backend-inference-backends.md) | The `InferenceBackend` trait, HTTP/NDJSON plumbing, and the engines (local: llama.cpp, vLLM; remote: vLLM) | `inference/backend/`, `inference/http/`, `inference/llama/`, `inference/vllm/`, `inference/llama_cpp/`, `inference/openai/`, `inference/vllm/`, `inference/vllm/`, `inference/generate/`, `inference/token_handler.rs`, `inference/chat/`, `commands/remote/` |
