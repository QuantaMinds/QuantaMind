...
executes it against a real MCP server — on **both** Ollama and llama.cpp.
...
maps 1:1 to `function.parameters`. Same shape feeds Ollama `/api/chat` and the
OpenAI `/v1` path.
...
lands in prose). `stream:false` (Ollama drops tool calls under streaming).

## Step 3 — Dispatch + execute + warn
`single_turn(backend, …)` matches `BackendKind`: Ollama `ollama_chat::
chat_with_tools` / llama.cpp `llama_chat::chat_with_tools` (both return the shared
`ChatResult`); MLX rejected (no native tool API). Each call → `execute_call` →
...
- **Live, REAL model on BOTH backends** (`bridge_single_turn…`):
  - **Ollama `qwen3.5:9b`** → emitted `read_text_file` → bridge read the seeded
...
**Real-world friction captured:** a locally-imported Ollama GGUF
