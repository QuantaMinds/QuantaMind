...
- **`qm cliff`: shipped** — the Context Stress Test headless (`cli/cliff.rs` wrapping the same
  ...
  collapsed / 11 inconclusive / 20 broken). Plus interactive pickers for `--collection` (the 15
  offered built-ins listed with tier/domain — 3 capability domains per tier + the 3 Category K
  probes; unlisted fixture collections are still accepted by id) and
