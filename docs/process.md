...
- **`v<semver>`** (e.g. `v0.2.3`) — the DESKTOP app. Fires
  `release-desktop.yml` (tauri-action → signed updater bundles → draft release
  → manual upload to `quantamind.co/releases/`, via `scripts/release.sh`).
- **`quantamind-v<semver>`** — the HEADLESS `qm` CLI. Fires
  `quantamind-release.yml` (generated by `dist` from `dist-workspace.toml`):
  6-target matrix, shell/powershell installers, sha256 checksums, GitHub
  artifact attestations, published (non-draft) GitHub Release.
- The patterns are mutually exclusive by construction (`v*` cannot match a
  `quantamind-` prefix and vice versa); dist additionally enforces
  `tag-namespace = "quantamind"`. Never edit `quantamind-release.yml` by hand —
  change `dist-workspace.toml` and run `dist generate`.
- dist refuses a tag whose version doesn't match `backend/Cargo.toml` — bump
  the manifests first (the `scripts/release.sh` bump block is the reference).
